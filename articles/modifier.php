...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($erreurs)) {
                ...
                if (move_uploaded_file($file['tmp_name'], $destination)) {
                    ...
                    if (!empty($article['image_url'])) {
                        $ancienFichier = realpath(__DIR__ . '/..') . DIRECTORY_SEPARATOR
    ...
    // ── Mise à jour en base uniquement si aucune erreur ──────────────────────
    if (empty($erreurs)) {
        $stmt = $db->prepare("
            UPDATE articles
            ...
            WHERE id = ?
        ");
        $stmt->execute([$titre, $desc, $contenu, $cat_id, $image_url, $id]);
        header('Location: liste.php');
        exit;
    }

    // Pré-remplir depuis POST si erreur de validation
...
<form method="POST" class="form-container" action="modifier.php?id=<?= $id ?>"
      id="form-modifier" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <textarea id="description_courte" name="description_courte"
                  rows="2"><?= htmlspecialchars($article['description_courte']) ?></textarea>
    </div>
    ...
    <div class="form-group">
        <label for="image">
            Changer l'image
            <small>(laisser vide pour conserver l'actuelle — JPEG, PNG, WebP, GIF — max 2 Mo)</small>
        </label>
        ...
        <?php if (!empty($article['image_url'])): ?>
            ...
            $cheminLocal = realpath(__DIR__ . '/..') . DIRECTORY_SEPARATOR
                         . str_replace('/', DIRECTORY_SEPARATOR, $article['image_url']);
            ?>
            <?php if (file_exists($cheminLocal)): ?>
                <div style="margin-bottom:10px;">
                    <p style="margin:0 0 4px;"><strong>Image actuelle :</strong></p>
        ...
        <div id="preview-container" style="display:none; margin-top:10px;">
            <p style="margin:0 0 4px;"><strong>Nouvelle image :</strong></p>
